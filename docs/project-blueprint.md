...
- **Format interpretation:** `@moldea.ai/core` validates canonical structure and relationships without returning repository bodies. It prepares deterministic inspection state, paged assignments and diagnostics, changed-path matching, and bounded per-agent adapter context. The public Repository Format specification remains its design authority.
