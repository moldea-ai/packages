...
Public packages are released automatically after their changes reach `main`. Pull requests invoke `CI` directly, while every `main` push invokes `Publish npm Packages` as its sole verification and release orchestrator; manual `CI` dispatch remains available for branch checks. The publish workflow requires every changed public project manifest to declare a stable version that is strictly greater than its base version when one exists. It selects changed public projects and every current repository version missing from npm, including a release left pending by an earlier failed or interrupted workflow, and calls the reusable CI workflow exactly once for every `main` commit, including successful no-op releases. It then publishes the exact checksummed tarballs in dependency order when releases are selected.

The coordinated skill `4.0.2` release adds a temporary fail-closed job to the reusable workflow when `release_build` is true. The job checks out the exact packages commit and full history separately from the frozen skill comparator, pins that comparator to one full commit, downloads the authoritative `public-package-tarballs` artifact, and compares it with immutable packages source baseline `9fa7a2ba4d3455b6310a62ece172571ab473f745` and tree `9eba53f4f54185432b04a4f9a4fac44543b5d24b`. A skipped, cancelled, or failed required comparison prevents the reusable workflow from succeeding, so no downstream tag or publication job can begin. The hosted result is an external release gate; the later skill attestation binds its deterministic inputs and comparator identity without claiming to store the hosted job outcome.
...
5. For the skill `4.0.2` package closure, verify the pinned compatibility bridge against the complete checksummed candidate artifact and exact candidate commit.
...
The `4.0.2` package compatibility job is temporary. Keep it fail-closed until the published package closure passes the final registry-backed comparison and the skill `4.0.2` compatibility attestation is complete. Remove the job and its frozen comparator checkout only in a later reviewed release after both conditions are recorded; do not remove it during package publication or in response to a transient hosted failure.
