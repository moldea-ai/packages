...
The selector also compares private workspace package graphs from both commits. A release-relevant private package change selects each public package that directly or transitively consumes it through private `dependencies` or `devDependencies`. Propagation stops at the first public package, so a compatible adapter change does not automatically select CLI. Reading both graphs catches moved, removed, or replaced private inputs; missing dependencies, duplicate package names, and private dependency cycles fail the check. Test-only files and nonshipped private documentation remain excluded. Shared implementation changes under `configs/vite/` or `configs/typescript/` select the public library-build consumers; config tests and prose do not. Every current public package uses those shared build inputs.
