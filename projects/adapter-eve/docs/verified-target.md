...
Positive agent evidence requires an uncollided flat or nested `agent.ts` that directly default-exports `defineAgent(...)`, uses `model` plus optional `description`, `defaultTools`, and `tool`, and resolves `model` to a supported static string. Before Eve `0.67.0`, a directly wired agent `outputSchema` is also supported. Eve `0.67.0` removes that agent definition field: an authored property or agent schema binding on a declaration confined to newer versions is a confirmed feature error, while a spanning declaration leaves the affected relationship and dependent agent evidence unverified. The `defaultTools` and `tool` options require eligible releases that define them. A newer version without an agent output schema retains positive agent evidence, and tool output schemas remain independently supported. Directory-backed local subagents use the same rule recursively.
