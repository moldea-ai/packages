...
export const CLAUDE_AGENT_SDK_ADAPTER_DIAGNOSTICS = Object.freeze({
  ...
  CLAUDE_AGENT_SDK_INSTRUCTION_LOADER_NOT_WIRED: {
    message: 'The declared instruction loader is not wired to the detected Claude Agent SDK agent.',
    severity: 'error',
  },
  ...
  CLAUDE_AGENT_SDK_PACKAGE_MANIFEST_INVALID: {
    message: 'The owning package manifest is invalid for Claude Agent SDK dependency detection.',
    severity: 'error',
  },
  ...
  CLAUDE_AGENT_SDK_SOURCE_SYNTAX_INVALID: {
    message: 'The referenced Claude Agent SDK source file contains invalid TypeScript syntax.',
    severity: 'error',
  },
  CLAUDE_AGENT_SDK_SOURCE_TEXT_INVALID: {
    message: 'The referenced Claude Agent SDK source file is not valid normalized text.',
    severity: 'error',
  },
  ...
  CLAUDE_AGENT_SDK_TOOL_NAME_MISMATCH: {
    message: 'The declared tool name does not match the detected Claude Agent SDK MCP tool name.',
    severity: 'error',
  },
  ...
  CLAUDE_AGENT_SDK_VERSION_UNSUPPORTED: {
    message: 'The observed Claude Agent SDK dependency range is disjoint from the supported range.',
    severity: 'error',
  },
...
export const createClaudeAgentSdkDiagnostic = (
  ...
  if (input.code === 'CLAUDE_AGENT_SDK_RUNTIME_RELATIONSHIP_UNVERIFIED') {
    const definition = CLAUDE_AGENT_SDK_ADAPTER_DIAGNOSTICS[input.code];
