...
describe('@moldea.ai/adapter-claude-agent-sdk public API', () => {
  test('exposes only the intended runtime symbol', () => {
    expect(Object.keys(publicApi)).toStrictEqual(['claudeAgentSdkAdapter']);
  });
  ...
  test('emits consumable public artifacts without private imports or test files', () => {
    ...
    expect(declaration).toContain('claudeAgentSdkAdapter');
    expect(declaration).not.toContain('CLAUDE_AGENT_SDK_ADAPTER_DIAGNOSTICS');
    expect(declaration).not.toContain('IClaudeAgentSdkAdapterDiagnosticCode');
