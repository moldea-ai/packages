...
describe('@moldea.ai/adapter-google-genai public API', () => {
  test('exposes only the intended runtime symbol', () => {
    expect(Object.keys(publicApi)).toStrictEqual(['googleGenAiAdapter']);
  });
  ...
  test('emits consumable public artifacts without private imports or test files', () => {
    ...
    expect(declaration).toContain('googleGenAiAdapter');
    expect(declaration).not.toContain('GOOGLE_GENAI_ADAPTER_DIAGNOSTICS');
    expect(declaration).not.toContain('IGoogleGenAiAdapterDiagnosticCode');
...
describe('published binding example', () => {
  ...
  test('establishes every documented relationship through the real adapter', async () => {
    const result = await createCore({ adapters: [publicApi.googleGenAiAdapter] }).validateProject({
