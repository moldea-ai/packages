...
describe('@moldea.ai/adapter-langchain public API', () => {
  test('exposes only the intended runtime symbol', () => {
    expect(Object.keys(publicApi)).toStrictEqual(['langChainAdapter']);
  });
  ...
  test('emits consumable public artifacts without private imports or test files', () => {
    ...
    expect(declaration).toContain('langChainAdapter');
    expect(declaration).not.toContain('LANGCHAIN_ADAPTER_DIAGNOSTICS');
    expect(declaration).not.toContain('ILangChainAdapterDiagnosticCode');
