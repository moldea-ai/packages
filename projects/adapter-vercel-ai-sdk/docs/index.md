...
The package is available. Its technical targets use TypeScript ESM and npm `ai >=7.0.66` with Repository Format version `1` and compatible Core `^5.0.0`.
...
The package exports only `vercelAiSdkAdapter`. It has no default export, configuration factory, SDK facade, parser export, public diagnostic registry, or mutable runtime state. The generated [API reference](https://packages.moldea.ai/adapters/vercel-ai-sdk/api/) derives that surface from the package export.

Start with the [complete binding example](https://packages.moldea.ai/adapters/vercel-ai-sdk/binding-example/) when connecting runtime source to canonical instructions, schemas, tools, or routing metadata. The same example ships locally as `docs/binding-example.md` in the installed package.
