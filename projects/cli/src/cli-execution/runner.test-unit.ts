...
const INSTALLED_PACKAGE_METADATA = Object.freeze({
  dependencies: Object.freeze({
    '@moldea.ai/adapter-anthropic': 'workspace:^5.0.0',
    '@moldea.ai/adapter-claude-agent-sdk': 'workspace:^4.0.0',
    '@moldea.ai/adapter-cloudflare-agents': 'workspace:^4.0.0',
    '@moldea.ai/adapter-eve': 'workspace:^4.0.0',
    '@moldea.ai/adapter-google-genai': 'workspace:^4.0.0',
    '@moldea.ai/adapter-langchain': 'workspace:^4.0.0',
    '@moldea.ai/adapter-langgraph': 'workspace:^4.0.0',
    '@moldea.ai/adapter-openai': 'workspace:^5.0.0',
    '@moldea.ai/adapter-openai-agents-sdk': 'workspace:^4.0.0',
    '@moldea.ai/adapter-vercel-ai-sdk': 'workspace:^4.0.0',
...
`,
  ...
  test('returns the exact installed version without dispatching a command', async () => {
    ...
    ).resolves.toStrictEqual({ exitCode: 0, stderr: '', stdout: '9.0.0\n' });
    expect(executeCommand).not.toHaveBeenCalled();
  });
  ...
  test('reports composition and maps failed command execution to a safe error', async () => {
    ...
    await expect(
      ...
      stderr: '',
      stdout:
        '{"cliVersion":"9.0.0","command":"inspect","error":{"code":"INTERNAL_ERROR","details":{},"message":"The command could not be completed.","path":null,"retryable":false,"source":"cli"},"result":null,"schemaVersion":5,"status":"error"}\n',
    });
  }, 30_000);
});
