...
describe('schema 4 CLI presentation formatters', () => {
  ...
  test('formats safe human and strict schema 4 JSON errors', () => {
    ...
    expect(JSON.parse(formatMoldeaCliJsonError(error, 'inspect', '7.1.0'))).toStrictEqual({
      cliVersion: '7.1.0',
      command: 'inspect',
      error,
      result: null,
  ...
  test('formats composition in human and schema 4 JSON forms', () => {
    ...
    expect(formatMoldeaCliHumanCompositionResult(result, '7.1.0')).toContain(
      'JSON output schema: 4',
    );
    expect(JSON.parse(formatMoldeaCliJsonCompositionResult(result, '7.1.0'))).toMatchObject({
      command: 'composition',
      result,
      schemaVersion: 4,
      status: 'valid',
  ...
  test('formats paginated validation diagnostics in both output modes', () => {
    ...
    expect(JSON.parse(formatMoldeaCliJsonValidateResult(result, '7.1.0'))).toMatchObject({
      command: 'validate',
