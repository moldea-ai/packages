...
The selected root may itself resolve through a symlink or junction; its resolved target becomes the fixed boundary. Stable descendant symlinks and junctions remain logical symlink entries and are not traversed. The reader rechecks the root and path ancestors during access, then revalidates regular-file identity before and after each range read. File opening does not follow the final symlink where the runtime supports it. A caller must trust the selected tree against hostile concurrent replacement between these path-based checks and opens. Entry metadata leaves `contentIdentity` null because stat identity cannot prove portable byte equality; comparisons use bounded file ranges instead.
